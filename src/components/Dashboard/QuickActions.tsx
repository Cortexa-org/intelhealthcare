...
import { MessageSquare, Calendar, Activity, LayoutGrid, History, UserSearch } from 'lucide-react';
...
export default function QuickActions({ user, onNavigate }: QuickActionsProps) {
  const careTimelineAction = {
    name: 'Care Timeline',
    description: 'Encounters, vitals, notes, meds, labs',
    icon: History,
    action: () => onNavigate('care-timeline'),
    color: 'bg-indigo-600 hover:bg-indigo-700',
  };

  const patientLookupAction = {
    name: 'Patient Lookup',
    description: 'Search & clinical summary',
    icon: UserSearch,
  ...
  const governanceAction = {
    name: 'Governance',
    description: 'Clinician feedback & governance dashboards',
    icon: LayoutGrid,
  ...
  const showGovernance = user?.role === 'clinician' || user?.role === 'admin';
  const actions = showGovernance ? clinicianActions : patientActions;
